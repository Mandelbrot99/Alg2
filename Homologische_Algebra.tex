...
\begin{proof}
	...
	\begin{enumerate}
		...
		\end{tikzcd} $ von $R$-Moduln. denn: 
			...
			"'$\subseteq$"' Sei $(y,z) \in \ker g \Ra \pi(z) = 0 \Ra z \in \im \iota,$ das heißt es existiert ein $ x \in L $  mit $ z = \iota(x) = -\iota(-x) \Ra (y,z) = (y, -\iota(-x)) = (y+\phi(x), 0) +(\phi(-x), -\iota(-x)) = (y+\phi(x), 0) = f(y+ \phi(x)) \in \im f.$
		\end{itemize}
...
\end{bem}
\begin{proof}
	Sei $ I \subseteq R $ ein Linksideal, $\phi: I \to Hom_{\Z}(R,M) $ $R$-linear. Nach 6.9, genügt es zu zeigen: $\phi$ lässt sich auf $R$ fortsetzen. Setze $$f: I \to M ,\quad a \mapsto \phi(a)(1)$$ Dann ist $f $ ist $\Z$-linear und für $r \in R , a \in I $ gilt: $ f(ra) = \phi(ra)(1)= (r\phi(a))(1) = \phi(a)(1 r) = \phi(a)(r)$. Da $M$ ein injektiver $\Z$-Modul, existiert eine $\Z$-lineare Abbilsung $g: R \to M $ mit $g\big|_y=f $. Wir setzen $\psi: R \to Hom_{\Z}(R,M), a \mapsto ag $. $\psi$ ist $R$-linear, und für $a\in I, r \in R $ ist $\psi(a)(r) =(ag)(r) = g(ra) = f(ra) = \phi(a)(r)$, das heißt $\psi\big|_{I}=\phi.$
\end{proof}
\begin{df}
...
\end{fo}
\begin{proof}
	$i) \Ra ii)$ NAch 6.19 existert ein kofreier $R$-Modul $N$, sodass $Q$ Untermodul von $N$ ist. Die exakte Folge 
	$$\begin{tikzcd}
...
\end{anm}
\begin{sa}[Lange exakte Kohomologiefolge]
	Sei $$\begin{tikzcd}
...
\begin{sa}[Hufeisenlemma]
	...
	0 \arrow{r}\arrow{d} & A'\arrow{d} \arrow{r} & I'^0\arrow{d} \arrow{r} & I'^1 \arrow{d} \arrow{r} & \ldots
	\end{tikzcd}$$
\end{sa}