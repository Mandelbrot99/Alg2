...
\begin{proof}
	...
	\end{tikzcd} $ exakt. 
	\begin{enumerate}
		\item Injektivität von $f_{*}^{M}: $ Sei $ \phi \in Hom_{R}(M,N')$ mit $f_{*}^{M}(\phi) = 0 \Ra f \circ \phi = 0 \overset{f injektiv}{\Ra} \phi = 0$, also $ \ker f_{*}^{M} = 0$
		\item $\im f_{*}^{M} = \ker g_{*}^{M},$ \\
		"$\subseteq$"' Es  ist $g_{*}^{M} \circ f_{*}^{M} = (g \circ f)_{*}^{M} = O_{*}^{M} = 0$, also $\im f_{*}^{M} \subseteq \ker g_{*}^{M},$ \\
		"$\supseteq$ "' 
		%hier fehlt was
	...
	\begin{enumerate}
		\item $f$ injektiv: Setze $M := \ker f , \iota: \ker f \to N' $ Inkusion $\Ra f_{*}^{M}(\iota)  = f \circ \iota = 0  \underset{f_{*}^{M} inj.}{\Ra} \iota = 0 \Ra \ker f = 0$
		\item $\im f = \ker g: $ \\
		"$\subseteq$"' Setze $M := N' \Ra 0 = 0_{*}^{M}(id_{N'}) = (g_{*}^{M} \circ f_{*}^{M}) (id_{N'}) = ((g \circ f)_{*}^{M})(id_{N'}) = g \circ f \circ id_{N'} = g \circ f $ \\
		"$\supseteq$ "'Setze $M := \ker g , \iota: \ker g \to N \Ra g_{*}^{M} (\iota) = g \circ \iota = 0 \Ra i \in \im f_{*}^{M} \Ra$ Es existiert ein $\phi: \ker g \to N' $ mit $ f \circ \phi = \iota. $ Somit: $ x \in \ker g \Ra x = \iota(x) = f(\phi(x)) \in \im f.$
	\end{enumerate}
\end{proof}
\begin{anm}
...
\begin{sa}
	...
	\begin{enumerate} [label= \roman*)]
		...
		%hier fehlt diagramm  1
		\item Jede kurze exakte Sequenz $\begin{tikzcd}
		0  \arrow{r} & L \arrow{r}{f} & M \arrow{r}{g} & P \arrow{r} & 0
		\end{tikzcd} $ von $R$-Moduln spaltet.
		\item Es gibt einen $R$-Modl $P'$, sodass $ P \bigoplus P' $ ein freier $R$-Modul ist (das heißt $P$ ist direkter Summand eines freien $R$-Moduls )
	\end{enumerate}
\end{sa}
\begin{proof}
...
\begin{proof}
	...
	$(iv) \Ra (iii) $ Sei $\iota : L \to M $ ein Monmorphismus, $ \phi: L \to Q $ ein Homomorphismus von $R$-Moduln. Setze $ S := \{ (\phi(x), \iota(x))| x \in L\} \subseteq Q \bigoplus M $(ist ein Untermodul). $M' := \QR{(Q \bigoplus M)}{S}, N := \QR{M}{\im \iota}, \pi: M \to n  $ kanonische Projektion. 
	\begin{enumerate}
