...
\begin{sa}\label{8.3}
	...
	\end{tikzcd}$ eine exakte Folge in $\AA$. Dann existieren natürliche Morphismen 
...
\end{proof}
\begin{df}\label{8.4}
	Sei $A\in \ObA$. Dann heißt $A$ \define{$F$-azylisch\index{azyklische Objekte}} $\defi R^iF(A) = 0$ für alle $i>1$.
\end{df}
\begin{bem}\label{8.5}
...
\end{proof}
\begin{anm}
	Die Theorie der Linksableitung rechtsexakter Funktoren lässt sich analog entwickeln: Sei $\AA$ eine abelsche Kategorie mit genügend vielen Projektiven, $\BB$ eine abelsche Kategorie, $F:\AA\to \BB$ ein rechtsexakter Funktor. Wir wählen für jedes $A\in \ObA$ eine projektive Auflösung $P_\bullet \to A$ und setzen 
...
\begin{df}
	...
	\begin{enumerate}
		...
		0\arrow{r} & A' \arrow{r} & B'\arrow{r} & C' \arrow{r} & 0
		\end{tikzcd} $$
		ein kommutatives Diagramm in $\AA$ mit exakten Zeilen, dann kommutiert 
		$$\begin{tikzcd}
...
\end{bsp}
\begin{df}
	Sei $H=(H^n)_{n\geq 0}: \A \to \BB $ ein $\delta$-Funktor. H heißt \define{universell} $\defi $ Für jeden $\delta$-Funktor $H'=(H^{'n})_{n\geq 0}: \A \to \BB $ setzt sich jede natürliche Transformation $ f^{\circ}: H^{\circ} \Ra H^{'\circ} $ eindeutig zueinem Homomorphismus von $\delta$-Funktoren fort, d.h. zu einer Familie $f=(f^{n})_{n \geq 0}$ von natürlichen Transformationen $f^{n}: H^{n} \Ra H^{'n} $ die auf naheliegende Weise mit den $\delta^{'}$ verträglich sind.
\end{df}
\begin{bem}\label{9.4}
...
\begin{proof}\label{10.3}
	\begin{enumerate}
		...
		%diagramm 
		$I^{0} $ injektiv, dann folgt mit \ref{6.11} $I^{0} $ ist teilbar $\Ra \coker \epsilon = \QR{I^0}{im \epsilon } $ teilbar, damit folt aus \ref{6.12} und da A ein HIR, dass der $\coker \epsilon $ injektiv ist.  Setze ..
		% nicht mitgekommen 
		\item Für -...
		%fehlt
