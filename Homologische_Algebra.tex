...
\begin{proof}
	...
	\end{tikzcd}$$
	(Nach dem Homomorphiesatz ist $k_{i-1}$ ein Mono-, $q_{i-1}$ ein Epi- und $\bar{d}_{i-1}$ ein Isomorphismus). Es ist $0= d_i \circ d_{i-1} = d_i \circ k_{i-1} \circ \bar{d}_{i-1} \circ q_{i-1}$ und, da $q_{i-1}$ Epi, $d_{i-1}$ Iso, folgt $d_i \circ k_{i-1} =0$. Es existiert ein $l_i : \im d_{i-1} \to \ker d_i$ mit $k_{i-1} = j_i \circ l_i$. $l_i$ ist ein Monomorphismus, da $k_{i-1} = j_i \circ l_i$ Monomorphismus.
\end{proof}
\begin{df}\label{7.3}
...
\begin{sa}\label{7.14}
	...
	sodass gilt: \begin{itemize}
		...
		\item Alle $I^i, i \leq 0,$ sind injektiv.
	\end{itemize}
