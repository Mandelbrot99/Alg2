...
\begin{anm}
	...
	\end{itemize}
\end{anm}
\begin{sa}[Hilbertscher Basissatz]
...
\end{proof}
\begin{df}
	Sei $M$ ein $R$-Modul. $M$ heißt "'artinsch"' $\defi$ für jede absteigende Kette $M_1 \supseteq M_2 \supseteq \ldots$ von Untermoduln von $M$ gibt es ein $n\in \N$ mit $M_i = M_n$ für alle $i\geq n$ ("'absteigende Kettenbedingung"').
\end{df}
\begin{df}
	$R$ heißt "'linksartinsch"' (bzw. "'rechtsartinsch"') $\defi R$ ist als Links- bzw. Rechtsmodul über sich selber artinsch. $R$ heißt "'artinsch"' $\defi R$ ist links- und rechtsartinsch.
\end{df}
\begin{bsp}
	\begin{enumerate}[label= \alph*)]
...
\end{fo}
\begin{df}
	Sei $M$ ein $R$-Modul. Dann heißt $M$ "'endlich koerzeugt"' $\defi $ für jede Familie $(M_i)_{i\in I}$ von Untermoduln von $M$ mit $\bigcap_{i\in I} M_i = 0$ existiert eine endliche Teilmenge $J\subseteq I$ mit $\bigcap_{j\in J} M_j = 0$
\end{df}
\begin{anm}
	\begin{itemize}
...
\begin{df}
	...
	\begin{itemize}
		\item einer Klasse $Ob\mathcal{C}$ von "Objekten"
			einer Menge $Mor_{\mathcal{C}}(A,B) $ von "Morphismen" für alle $A,B \in Ob\mathcal{C}$
		\item einer Verknüpfung $\circ : Mor_{\mathcal{C}}(B,C) \times Mor_{\mathcal{C}}(A,B) \to Mor_{\mathcal{C}}(A,C) $ für alle $A,B,C \in Ob\mathcal{C}$
	\end{itemize}
	wobei folgende Axiome gelten:
	\begin{enumerate}
		\item[$(K1)$] $Mor_{\mathcal{C}}(A,B) \cap Mor_{\mathcal{C}}(A',B') = \emptyset$, falls $A \neq A'$ oder $B \neq B'$
		\item[$(K2)$] Für alle $A,B,C,D \in Ob\mathcal{C}, f \in Mor_{\mathcal{C}}(A,B), g \in Mor_{\mathcal{C}}(B,C), h \in Mor_{\mathcal{C}}(C,D)$ gilt:
		$$ h \circ (g \circ f) = (h \circ g) \circ f \quad (\text{Assoziativität})$$
		\item[$(K3)$] für jedes $ A \in Ob\mathcal{C}$ existiert ein Morphismus $id_A \in Mor_{\mathcal{C}}(A,A)$, sodass für alle $B \in Ob\mathcal{C}, f \in Mor_{\mathcal{C}}(A,B), g \in Mor_{\mathcal{C}}(B,A)$ gilt: $$f \circ id_A = f, \ id_A \circ g = g $$.
	\end{enumerate}
\end{df}
\begin{anm}
	\begin{itemize}
		\item Man sagt "Klasse"' statt Menge, um Paradoxien, wie " die Menge aller Mengen" zu vermeiden.
		\item Trotzdem schreiben wir $A \in Ob\mathcal{C}$ um zu sagen dass $A$ zu $Ob\mathcal{C}$ gehört (und werden $Ob\mathcal{C}$ im Folgenden wie eine Menge behandeln).
...
\begin{bsp}
	\begin{itemize}
		...
		\item $Ob\mathcal{C}=\{\ast\}, Mor_{\mathcal{C}}(\ast,\ast) := M $, wobei $M$ Monoid, $\circ = $ Verknüpfung in $M$.
	\end{itemize}
\end{bsp}
\begin{df}
	Sei $\mathcal{C}$  eine Kategorie. Die zu $\mathcal{C}$ "duale Kategorie '" $\mathcal{C}^{op}$ ist die Kategorie mit: 
	\begin{itemize}
		\item $Ob\mathcal{C}^{op} = Ob\mathcal{C}$, $Mor_{\mathcal{C}^{op}}(A,B) := Mor_{\mathcal{C}}(B,A)$ für $A,B \in Ob\mathcal{C}^{op} = Ob\mathcal{C}$
		\item $\circ_{op}: Mor_{\mathcal{C}^{op}}(A,B) \times Mor_{\mathcal{C}^{op}}(B,C) \to Mor_{\mathcal{C}^{op}}(A,C) $ mit $(f,g) \mapsto f \circ g $ für $A,B,C \in Ob\mathcal{C}$
	\end{itemize}
\end{df}
\begin{anm}
	\begin{itemize}
...
\end{anm}
\begin{df}
	Seien $\mathcal{C}, \mathcal{D} $ Kategorien. Ein "(kovarianter) Funktor" $F: \mathcal{C} \to \mathcal{D} $ besteht aus einer Abbildung $$ Ob\mathcal{C} \to Ob\mathcal{D}, \quad A \mapsto FA$$
	und Abbildungen: $$ Mor_{\mathcal{C}}(A,B) \to Mor_{\mathcal{D}}(FA,FB), \quad f \mapsto F(f) $$ für alle $A,B \in Ob\mathcal{C}$, sodass gilt:
	\begin{enumerate}
		\item[(F1)] $F(g \circ f) = F(g) \circ F(f) $ für alle $ f \in Mor_{\mathcal{C}}(A,B), g\in Mor_{\mathcal{C}}(B,C), \ A,B,C \in Ob\mathcal{C}$
		\item[(F2)] $F(id_A) =id_{FA} $ für alle $A \in Ob\mathcal{C}.$
	\end{enumerate}
\end{df}
\begin{bsp}
	\begin{enumerate} [label= \alph*)]
		\item Vergiss-Funktoren, zum Beispiel: $R-Mod\to Mengen$, $R-Mod \to \Z-Mod$, ...
		\item Sei $\mathcal{C}$ eine Kategorie $\Ra$ Jedes Objekt $X \in Ob\mathcal{C}$ induziert einen Funktor $$Mor_{\mathcal{C}}(X,-): \mathcal{C} \to \text{Mengen}, \quad A \mapsto Mor_{\mathcal{C}}(X,A) $$
		Für $f \in Mor_{\mathcal{C}}(A,B) $ ist hierbei $f_{\ast}^{X} := Mor_{\mathcal{C}}(X,-)(f)$ gegeben durch $$ f_{\ast}^{X}: Mor_{\mathcal{C}}(X,A) \to Mor_{\mathcal{C}}(X,B), \quad g \mapsto f \circ g$$
		\item Sei $M \in R-Mod$ $\Ra Hom_R(M,-): R-Mod \to \Z-Mod, N \mapsto Hom_R(M,N) $ ist ein Funktor.
	\end{enumerate}
\end{bsp}
\begin{df}
	Seien $\mathcal{C}, \mathcal{D}$ Kategorien. Ein "(kontavarianter) Funktor" $F$ von $\mathcal{C}$ nach $\mathcal{D}$ ist ein Funktor $F: \mathcal{C}^{op} \to \mathcal{D} $, das heißt besteht aus einer Abbildung $$ Ob\mathcal{C} \to Ob\mathcal{D}, \quad A \mapsto FA$$
	und Abbildungen: $$ Mor_{\mathcal{C}}(A,B) \to Mor_{\mathcal{D}}(FB,FA), \quad f \mapsto F(f) $$ für alle $A,B \in Ob\mathcal{C}$, sodass gilt:
	\begin{enumerate}
...
\begin{bsp}
		\begin{enumerate} [label= \alph*)]
		\item Sei $\mathcal{C}$ eine Kategorie $\Ra$ Jedes Objekt $Y \in Ob\mathcal{C}$ induziert einen kontravarianten Funktor $$Mor_{\mathcal{C}}(-,Y): \mathcal{C} \to \text{Mengen}, \quad A \mapsto Mor_{\mathcal{C}}(A,Y) $$
		Für $f \in Mor_{\mathcal{C}}(A,B) $ ist hierbei $f_{Y}^{\ast} := Mor_{\mathcal{C}}(-,Y)(f)$ gegeben durch $$ f_{Y}^{\ast}: Mor_{\mathcal{C}}(B,Y) \to Mor_{\mathcal{C}}(A,Y), \quad g \mapsto g \circ f$$
...
\end{anm}
\begin{df}
	Seien $\mathcal{C},\mathcal{D} $ Kategorien. "Das Produkt" $\mathcal{C} \times \mathcal{D} $ ist diejenige Kategorie mit $ Ob(\mathcal{C} \times \mathcal{D}) = Ob\mathcal{C} \times \mathcal{D} $ und $ Mor_{\mathcal{C} \times \mathcal{D}}((A_1,B_1),(A_2,B_2)) = Mor_{\mathcal{C}}(A_1,A_2) \times Mor_{\mathcal{D}}(B_1,B_2)$ und "komponentenweisen $\circ"$.
\end{df}
\begin{df}
...
\end{bsp}
\begin{df}
	Sei $\mathcal{C}$ eine KAtegorie, $ A,B \in Ob\mathcal{C}, f: A \to B $
	\begin{enumerate}
		\item[] $f$ heißt "Monomorphismus" $\defi$ Für alle $C \in Ob\mathcal{C}, g_1,g_2: C \to A $ gilt: $f \circ g_1 = f \circ g_2 \Ra g_1 = g_2$ $\Lra$ Für alle $C \in Ob\mathcal{C} ist f_{\ast}^{C}: Mor_{\mathcal{C}}(C,A) \to Mor_{\mathcal{C}}(C,B)$ injektiv.
		\item[] $f$ heißt "'Epimorphismus" $\defi$ Für alle $C \in Ob\mathcal{C}, g_1,g_2: B \to C $ gilt: $g_1 \circ f = g_2 \circ f \Ra g_1 = g_2$ $\Lra$ Für alle $C \in Ob\mathcal{C} ist f_{C}^{\ast}: Mor_{\mathcal{C}}(B,C) \to Mor_{\mathcal{C}}(A,C)$ injektiv.
		\item[] $f$ heißt "'Isomorphismus" $\defi$ Es existiert ein $g:b \to A $ mit $ f\circ g = id_B $ und $ g \circ f = id_A.$
	\end{enumerate}
\end{df}
\begin{anm}
...
\end{anm}
\begin{bem}
	Sei $\mathcal{C}$ eine Kategorie, $A,B \in Ob\mathcal{C}, f:A \to B $ ein Isomorphismus. Dann ist $f$ ein Monomorphismus und Ein Epimorphismus.
	\begin{proof}
		Seien $ C \in Ob\mathcal{C} , g_1,g_2:C \to A $ mit $ f \circ g_1 = f \circ g_2 \Ra f^{-1} \circ (f \circ g_1) = f^{-1} \circ (f \circ g_2) \Ra (f^{-1} \circ f) \circ g_1 = (f^{-1} \circ f) \circ g_2 \Ra g_1=g_2 \Ra f $ Monomorphimus. Analog wird gezeigt dass $f$ ein Epimorphimus.
...
\begin{bsp}
	\begin{enumerate} [label= \alph*)]
		...
		\item Sei $\mathcal{C} = Ringe, f:\Z \to \Q $ Inklusion. $f$ ist ein Monomorphismus und ein Epimorphimus (Achtung, denn: Für $g_1,g_2: \Q \to R$ Ringhomomorphismus ist ein Ring $R$ mit $g_1 \circ f = g_2 \circ f, $ das heißt $ g_1\big|_{\Z} = g_2\big|_{\Z} $ folgt $ g_1 = g_2 $ wegen der Universellen Eigenschaft von $\Q$ als Quotientenkörper von $\Z$), aber kein Isomorphismus. Insbesondere ist ein Epimorphismus in $\mathcal{C} $ im obigen Sinne ("kategorieller Epimorphismus") nicht dasselbe wie ein surjektiver Ringhomomorphismus.
	\end{enumerate}
\end{bsp}
