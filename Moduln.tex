...
\begin{sa}[Isomorphiesaätze]\label{sa1.13}Sei $M$ ein $R$-Modul, $N_1, N_2 \subseteq M$ Untermoduln. Dann gilt:
	\begin{enumerate}[label=\alph*)]
		...
		\item Ist $ N_2 \subseteq N_1$, so ist
		 $$\QR{M/N_2}{N_1/N_2} \overset{\sim}{\longrightarrow} \QR{M}{N_1}\qquad (x+N_2)+\QR{N_1}{N_2} \mapsto x+N_1$$
		 ein Isomorphismus.
	\end{enumerate}
\end{sa}
\begin{sa}\label{1.14}
...
\end{proof}
\begin{df}\label{1.23}
	Sei $A$ ein kommutativer Ring, $M$ ein freier $A$-Modul mit endlicher Basis. Die Kardinalität dieser Basis heißt der \define{Rang\index{Rang eines Moduls}} von M (unabhängig von der Wahl einer endlichen Basis nach \ref{1.22})
\end{df}
\newpage
...
\begin{sa}\label{2.2}
	...
	\end{enumerate}
	Ist eine dieser äquivalenten Bedingungen erfüllt, sagt man, das die kurze exakte Sequenz "'spaltet"'. In diesem Fall gilt: $M \cong M' \oplus M''$. Der Homomorphismus $s$ heißt ein "'Schnitt\index{Schnitt einer exakten Sequenz}"' von $g$.
\end{sa}
\begin{proof}
...
\end{sa}
\begin{proof}
	ohne Einschränkung gelte $n_1 \geq n_2  \Ra R^{n_2} \simeq R^{n_1} \simeq R^{n_2} \oplus R^{n_1 - n_2} $. Wegen $R^{n_2}$ noethersch, folgt mit \ref{3.10} : $R^{n_1 - n_2} =0$, also $ n_1=n_2$
\end{proof}
\begin{anm}
	\begin{itemize}
