...
\end{proof}
\begin{anm}
	Erinnerung an LA2 (29.15) Sei $M$ ein $A$-Modul. $$ T(M) := \{ x \in M | \text{es existiert ein} a \in A, a \text{kein Nullteiler, mit } ax = 0 \} $$ \define{Torsionsmodul von $M$}. \\
	$M$ heißt \define{torsionsfrei} $\defi T(M) = \{0\}$
\end{anm}
...
\begin{proof}
	...
	$(iv) \Ra (iii) $ \\
	Sei $N$ ein $A$-Modul mit $N \otimes_A M = 0 \Ra id_N \otimes_A id_M: N \otimes_AM \to N \otimes_A M $ ist die Nullabildung. Dann folgt mit (iv) $id_n = 0 \Ra N = 0 $. \\
	$(iii) \Ra (i) $ \\
	Sei $\begin{tikzcd} N^{'} \arrow{r} & N \arrow{r} & N^{''} \end{tikzcd}$ eine Sequenz von $A$-Moduln. \\
	Zu zeigen ist: $\begin{tikzcd} N^{'} \arrow{r}{f} & N \arrow{r}{g} & N^{''} \end{tikzcd}$ exakt $\Lra$ $\begin{tikzcd} N^{'} \otimes_A M  \arrow{r}{f \otimes id_M} & N \otimes_A M  \arrow{r}{g \otimes id_M} & N^{''} \otimes_A M  \end{tikzcd}$ exakt. \\
	...
	$"\supseteq " $ Wir betrachten die Sequenzen $\begin{tikzcd} N^{'} \arrow{r}{f|^{\im f}} & \im f \arrow{r} & 0 \end{tikzcd}$ \\
	...
	Diese sind exakt (wobei $\iota_1, \iota_2$ Inklusionen). \\
	Es ist $\im ((\iota_a \otimes_A id_M) \circ (\iota_i \otimes_A id_M) \circ (f|^{\im f} \otimes_A id_M )) = \im(f \otimes_A id_M) = \ker(g \otimes_A id_M) = \ker(g|^{\im g} \otimes_A  id_M) = \im(\iota_2 \otimes_A id_M)$\\
	 (Letzes Gleichheitszeichen aus der Flachheit von $M$ ). \\
	 Wegen $M$ flach ist $\iota_2 \otimes id_M $ injektiv ,woraus resultiert $\im ((\iota_1 \otimes id_M) \circ (f|^{\im f} \otimes id_M)) = \ker g \otimes_A M \\
	 \Ra (\iota_1 \otimes id_M ) \circ (f|^{\im f} \otimes id_M) $ surjektiv \\
	 Woraus folgt dass $\iota_1 \otimes id_M $ surjektiv ist. Da $M$ flach ist, erhalten wir die exakte Sequenz $$ \begin{tikzcd} 0 \arrow{r}  & \im f \otimes_A M \arrow{r}{\iota_1 \otimes id_M } & \ker g \otimes_A M \arrow{r} & (\QR{\ker g}{\im f}) \otimes_A M \arrow{r} & 0 \end{tikzcd}$$
	 Daraus resultiert: $(\QR{\ker g}{\im f}) \otimes_A M = 0 $ und mit (ii), dass $\QR{\ker g}{\im f} = 0$. Das heißt $\ker g = \im f $.
\end{proof}
\begin{bsp}
	\begin{enumerate} [label= \alph*)]
